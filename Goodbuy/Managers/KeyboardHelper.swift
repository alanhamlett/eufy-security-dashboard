...
public class KeyboardHelper: NSObject {
    ...
    public func removeDelegate(delegate: KeyboardHelperDelegate) {
        var index = 0
        for weakDelegate in delegates {
            // Reuse any existing slots that have been deallocated.
            if let d = weakDelegate.delegate, d === delegate {
                return
            }
            index += 1
        }
    ...
    func height() -> CGFloat {
        guard let window = KeyWindow, let view = window.rootViewController?.view else { return 0 }
        if let keyboardHeight = currentState?.intersectionHeightForView(view: view) {
